...
const HeartPage = () => {
    ...
    if (file && file.type === "application/pdf") {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/pdf/heart-scraper`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
        ...
        setFormData({
          ...
          fastingBloodSugar: ["1", "yes", "positive"].includes(
            String(data["Fasting blood sugar > 120 mg/dl"] || "").toLowerCase()
          )
            ? "1"
            : "0",
          restingECG: data["Resting Electrocardiographic Results"] || "",
          maxHeartRate: data["Maximum Heart Rate Achieved"] || "",
          exerciseInducedAngina: ["1", "yes", "positive"].includes(
            String(data["Exercise Induced Angina"] || "").toLowerCase()
          )
            ? "1"
            : "0",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/predict/heart-pred`,
        {
          method: "POST",
          headers: {
  ...
  const generateDynamicPDF = async () => {
    ...
    try {
      await downloadSynapXReport({
        ...
        fileName: "synapx-heart-disease-report.pdf",
      });
